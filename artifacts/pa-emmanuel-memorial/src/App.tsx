        ...
        <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:gap-24">
          ...
          <div className="space-y-10">
            ...
            <div className="grid gap-8 sm:grid-cols-2">
              <div className="flex gap-4" data-testid="event-venue">
                <MapPin className="mt-1 size-5 shrink-0 text-accent" strokeWidth={1.5} />
                <div><p className="mono text-[10px] uppercase tracking-[.2em] text-muted-foreground">Venue</p><p className="mt-2 text-sm leading-6">{event.venue}</p>{event.directionsUrl && <a className="mt-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[.1em] text-primary underline decoration-accent underline-offset-4" href={event.directionsUrl} target="_blank" rel="noreferrer" data-testid="link-directions">Open directions <ArrowRight className="size-3" /></a>}</div>
              </div>
              <div className="flex gap-4" data-testid="event-dress-code">
                <Sparkles className="mt-1 size-5 shrink-0 text-accent" strokeWidth={1.5} />
                <div><p className="mono text-[10px] uppercase tracking-[.2em] text-muted-foreground">Dress code</p><p className="mt-2 text-sm leading-6">{event.dressCode}</p></div>
              </div>
            </div>
            {event.clothingImageUrl && <img src={event.clothingImageUrl} alt="Suggested clothing for the service" className="mt-2 h-48 w-full object-cover grayscale-[.2]" data-testid="img-clothing" />}
          </div>
        </div>
      </div>
    </section>
    ...
    <div className="paper-grain min-h-[100dvh] bg-background">
      ...
      <main className="mx-auto max-w-5xl px-5 pb-20 pt-12 sm:px-8 lg:px-12">
        ...
        <section className="grid gap-10 border-x border-b border-border p-7 sm:grid-cols-[1fr_.8fr] sm:p-12">
          ...
          <div className="border-l border-accent pl-6">
            <p className="mono text-[10px] uppercase tracking-[.2em] text-muted-foreground">Gathering details</p>
             <div className="mt-5 space-y-6">
              ...
              <div><p className="text-xs uppercase tracking-[.1em] text-muted-foreground">Venue</p><p className="mt-1 text-sm leading-6">{event.venue}</p></div>
            </div>
         ...
         <div className="flex flex-col gap-4 border-b border-border px-7 py-6 sm:flex-row sm:items-center sm:justify-between sm:px-12"><p className="text-sm text-muted-foreground">Keep this invitation close for arrival.</p><div className="flex flex-wrap gap-4"><a href={event.directionsUrl} target="_blank" rel="noreferrer" data-testid="link-invite-directions" className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[.12em] text-primary"><MapPin className="size-4 text-accent" /> Directions</a><button type="button" data-testid="button-copy-invitation" onClick={() => navigator.clipboard?.writeText(invite.invitationUrl)} className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[.12em] text-primary"><Copy className="size-4 text-accent" /> Copy invitation link</button></div></div>
      </main>
    </div>
  );
}
...
function CheckInPage() {
  ...
  return (
    <div className="paper-grain min-h-[100dvh] bg-background">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-6 sm:px-8 lg:px-12"><Wordmark /><Link href="/" data-testid="link-checkin-home" className="inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.17em] text-muted-foreground"><ChevronLeft className="size-4" /> Back to invitation</Link></div>
      <main className="mx-auto max-w-4xl px-5 pb-20 pt-10 sm:px-8 lg:px-12">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_.9fr]">
          <section className="border border-border bg-card p-6 sm:p-9" data-testid="section-manual-checkin">
            ...
            {checkIn.data && <div className={`mt-6 border p-5 ${checkIn.data.result === 'invalid' ? 'border-destructive/30 bg-destructive/5' : 'border-accent/50 bg-accent/10'}`} data-testid="status-checkin-result"><p className="mono text-[10px] uppercase tracking-[.18em]">{checkIn.data.result === 'invalid' ? 'Not recognised' : checkIn.data.result === 'already_checked_in' ? 'Already checked in' : 'Arrival recorded'}</p><p className="serif mt-2 text-2xl">{checkIn.data.guestName ? `Welcome, ${checkIn.data.guestName}.` : 'Please check the code and try again.'}</p>{checkIn.data.rsvpStatus && <p className="mt-2 text-xs text-muted-foreground">RSVP: {statusLabel(checkIn.data.rsvpStatus)}</p>}</div>}
          </section>
...
function AdminLogin({ onLoggedIn }: { onLoggedIn: () => void }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    login.mutate({ data: { username, password } }, { onSuccess: (session) => { queryClient.setQueryData(getGetAdminSessionQueryKey(), session); onLoggedIn(); } });
  };
  return (
    <div className="paper-grain grid min-h-[100dvh] bg-primary lg:grid-cols-[.9fr_1.1fr]">
      ...
      <div className="flex items-center bg-background px-5 py-14 sm:px-12"><div className="w-full max-w-md"><div className="mb-12 lg:hidden"><Wordmark /></div><p className="mono text-[10px] uppercase tracking-[.22em] text-muted-foreground">Family workspace</p><h1 className="serif mt-4 text-5xl text-primary">Welcome back.</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">Sign in to manage invitations and welcome every guest.</p><form onSubmit={submit} className="mt-10 space-y-5"><div><label htmlFor="admin-username" className="mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">Username</label><input id="admin-username" value={username} onChange={(event) => setUsername(event.target.value)} required data-testid="input-admin-username" className="mt-2 h-12 w-full border border-input bg-card px-4 outline-none focus:border-primary" /></div><div><label htmlFor="admin-password" className="mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">Password</label><input id="admin-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} required data-testid="input-admin-password" className="mt-2 h-12 w-full border border-input bg-card px-4 outline-none focus:border-primary" /></div>{login.isError && <p className="text-sm text-destructive" data-testid="status-login-error">{errorMessage(login.error)}</p>}<button type="submit" disabled={login.isPending} data-testid="button-admin-login" className="flex h-12 w-full items-center justify-center gap-2 bg-primary text-xs font-semibold uppercase tracking-[.16em] text-primary-foreground">{login.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <ArrowRight className="size-4 text-accent" />} Enter workspace</button></form><Link href="/" data-testid="link-admin-back" className="mt-8 inline-flex items-center gap-2 text-xs text-muted-foreground"><ChevronLeft className="size-4" /> Return to public invitation</Link></div></div>
    </div>
  );
}
...
function GuestQrDialog({ guest, onClose }: { guest: Guest; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-40 grid place-items-center bg-primary/70 p-4" role="dialog" aria-modal="true" data-testid="dialog-guest-qr"><div className="w-full max-w-md border border-border bg-background p-6 text-center shadow-xl sm:p-8"><div className="flex items-start justify-between text-left"><div><p className="mono text-[10px] uppercase tracking-[.18em] text-muted-foreground">Private invitation code</p><h2 className="serif mt-2 text-3xl text-primary">{guest.name}</h2></div><button type="button" onClick={onClose} data-testid="button-close-qr-dialog"><X className="size-5 text-muted-foreground" /></button></div>{qrDataUrl ? <img src={qrDataUrl} alt={`QR code for ${guest.name}`} className="mx-auto mt-7 w-full max-w-[280px] border border-border p-3" data-testid="img-guest-qr" /> : <div className="mx-auto mt-7 grid aspect-square max-w-[280px] place-items-center border border-border"><LoaderCircle className="size-6 animate-spin text-accent" /></div>}<p className="mt-5 break-all font-mono text-xs text-muted-foreground" data-testid="text-guest-token">{guest.token}</p><p className="mt-2 break-all text-xs text-muted-foreground" data-testid="text-guest-invitation-url">{guest.invitationUrl}</p><div className="mt-6 grid grid-cols-2 gap-2"><a href={qrDataUrl || undefined} download={`invitation-${guest.name.replace(/\s+/g, '-').toLowerCase()}.png`} onClick={(event) => { if (!qrDataUrl) event.preventDefault(); }} data-testid="button-download-qr" className="border border-primary px-3 py-3 text-xs font-semibold uppercase tracking-[.11em]">Download QR</a><button type="button" onClick={print} disabled={!qrDataUrl} data-testid="button-print-qr" className="bg-primary px-3 py-3 text-xs font-semibold uppercase tracking-[.11em] text-primary-foreground disabled:opacity-50">Print QR</button></div></div></div>;
}
